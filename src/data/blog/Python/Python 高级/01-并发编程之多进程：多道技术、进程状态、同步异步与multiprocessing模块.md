...
进程（Process）是程序的一次执行实例，也是操作系统分配资源和调度执行的基本单位。程序是静态的代码和数据，进程则是动态运行中的实体，会经历创建、就绪、运行、阻塞和终止等状态。

一个进程通常具备以下特征：

1. **动态性**：进程有创建、运行、阻塞、退出的生命周期。
2. **独立性**：每个进程默认拥有独立地址空间，一个进程内的数据修改不会直接影响另一个进程。
3. **并发性**：多个进程可以在操作系统调度下交替运行，在多核 CPU 上也可以真正并行。
4. **异步性**：不同进程按各自速度推进，执行顺序受调度器、I/O 和优先级等因素影响。
5. **结构性**：进程不仅包含代码和数据，还包含进程控制块（PCB）中保存的 PID、状态、寄存器、打开文件和调度信息。

常见调度思路包括先来先服务、短作业优先、时间片轮转和优先级调度。Python 层面不直接控制操作系统调度算法，但理解这些概念有助于判断为什么进程会切换、阻塞或长时间得不到 CPU。

进程间通信（IPC）通常通过管道、队列、共享内存、信号量、套接字等机制完成。后文的 `multiprocessing.Queue`、`Pipe`、进程池等内容，都是 Python 对这些能力的封装。
