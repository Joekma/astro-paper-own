...
<!-- snippet: id=design-patterns-observer-01 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class AlarmSensor:
    def run(self):
        print("Alarm Ring...")
...
<!-- snippet: id=design-patterns-observer-02 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class Observer:
    def update(self):
        pass
...
<!-- snippet: id=design-patterns-observer-03 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class Observed:
    observers = []
    action = ""
...
<!-- snippet: id=design-patterns-observer-04 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-observer-05 mode=display python=3.12-3.14 deps=stdlib -->
```text
Alarm Got: On Fire!
Alarm Ring...
...
观察者模式定义了对象间一种一对多的依赖关系，当被观察者状态改变时，所有依赖于它的对象都会得到通知并自动更新。它常用于消息交换和多级触发场景。观察者与被观察者之间是抽象耦合的，可以方便地实现广播功能，但也可能带来系统效率的浪费。
