...
description: '深入讲解设计模式中的装饰器模式的原理和实践应用。'
tags:
  - Python
  - 设计模式
draft: false
...
<!-- snippet: id=design-patterns-decorator-01 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class Beverage:
    name = ""
    price = 0.0
...
<!-- snippet: id=design-patterns-decorator-02 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-decorator-03 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-decorator-04 mode=display python=3.12-3.14 deps=stdlib -->
```text
Name:coke
Price:4.0
Name:coke +ice
...
装饰器模式和代理模式非常相似，可以认为，装饰器模式就是代理模式的一个特殊应用，两者的共同点是都具有相同的接口，不同点是侧重点不同：代理模式侧重对主题类的过程的控制，而装饰模式则侧重对类功能的加强或减弱。
...
<!-- snippet: id=design-patterns-decorator-05 mode=compile python=3.12-3.14 deps=stdlib -->
```python
def log(func):
    def wrapper(*args, **kw):
...
<!-- snippet: id=design-patterns-decorator-06 mode=display python=3.12-3.14 deps=stdlib -->
```text
call now():
2016-12-04
```
...
<!-- snippet: id=design-patterns-decorator-07 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class LogManager:
    @staticmethod
    def log(func):
...
<!-- snippet: id=design-patterns-decorator-08 mode=display python=3.12-3.14 deps=stdlib -->
```text
Visit Func getName
Name:coke
Visit Func getPrice
...
装饰器模式动态地给一个对象添加一些额外的职责，在增加功能方面比生成子类更为灵活。Python的装饰器模式是实现AOP（面向切面编程）的一种简洁方式，可以将重复的操作行为提取出来进行统一管理和维护。但也要注意，多层装饰器的调试和维护会比较困难。
