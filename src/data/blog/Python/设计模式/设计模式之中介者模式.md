...
<!-- snippet: id=design-patterns-mediator-01 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class Colleague:
    mediator = None
...
<!-- snippet: id=design-patterns-mediator-02 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-mediator-03 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-mediator-04 mode=display python=3.12-3.14 deps=stdlib -->
```text
PURCHASE:Bought 300
...
中介者模式通过引入一个中介对象来封装一系列的对象交互，使各对象不需要显式地互相作用，从而使其耦合松散，并可以独立地改变它们之间的交互。它适用于出现网状结构的设计场景，可以将网状结构简化为星形结构。但也要注意，中介者本身的复杂性可能会很大。
