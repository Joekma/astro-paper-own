...
<!-- snippet: id=design-patterns-visitor-01 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class Medicine:
    name = ""
    price = 0.0
...
<!-- snippet: id=design-patterns-visitor-02 mode=compile python=3.12-3.14 deps=stdlib -->
```python
class Visitor:
    name = ""
...
药品类中有一个accept方法，其参数是个visitor；而工作人员就是从Visitor类中继承而来的，也就是说，他们就是Visitor，都包含一个visit方法，其参数又恰是medicine。药品作为处理元素，依次允许（Accept）Visitor对其进行操作，这就好比是一条流水线上的一个个工人，对产品进行一次次的加工。整个业务流程还差一步，即药方类的构建（流水线大机器）：

<!-- snippet: id=design-patterns-visitor-03 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-visitor-04 mode=compile python=3.12-3.14 deps=stdlib -->
```python
...
<!-- snippet: id=design-patterns-visitor-05 mode=display python=3.12-3.14 deps=stdlib -->
```text
...
<!-- snippet: id=design-patterns-visitor-06 mode=compile python=3.12-3.14 deps=stdlib -->
```python
def max_num(x, y, z):
...
<!-- snippet: id=design-patterns-visitor-07 mode=display python=3.12-3.14 deps=stdlib -->
```text
Traceback (most recent call last):
  File "example.py", line 786, in <module>
    print(max_num(1, 2, 4))
...
访问者模式封装了一些作用于某种数据结构中的各元素的操作，可以在不改变数据结构的前提下定义新的操作。它通过双分派机制实现了元素和操作的解耦，适用于需要遍历不同对象并执行不同操作的场景。但也要注意，访问者可能会了解到元素的细节，这与最小隔离原则有所冲突。
