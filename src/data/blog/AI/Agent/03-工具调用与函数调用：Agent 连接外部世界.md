...
**贯穿场景**：Agent 可以读取项目状态，但只有经审批才能写入任务文件。
...
![一个可用工具 Schema 必须声明什么](./images/agent-03-02-tool-schema-anatomy.png)
...
![为什么模型提议不等于工具执行](./images/agent-03-01-proposal-vs-execution.png)
...
![工具调用从生成参数到观察回写如何流转](./images/agent-03-03-tool-call-sequence.png)

![哪些工具可以并行，哪些必须串行](./images/agent-03-06-parallel-tool-calls.png)
...
![审批界面必须向用户展示哪些细节](./images/agent-03-07-approval-detail.png)
...
![工具输出中的指令为什么不能提权](./images/agent-03-08-untrusted-tool-output.png)

MCP 规范明确提醒，工具描述和注释也应被视为不可信输入，除非来自可信服务端。工具描述可能诱导模型做超出预期的事情，Host 侧仍应掌握最终授权。

## 错误恢复

![工具结果如何同时表达数据与错误语义](./images/agent-03-04-structured-tool-result.png)
...
## 工程补全：Tool Contract 与副作用边界

### 接口与数据契约

- 请求包含 tool_call_id、name、arguments 和 idempotency_key
- 响应使用 ok、data、error.code、error.retryable 和 metadata
- 并行调用只允许在工具互不依赖且不竞争写入时开启

### 失败路径、终止与恢复

![写工具超时后如何避免重复副作用](./images/agent-03-05-idempotency-timeout.png)

- 参数校验失败时返回结构化错误，不执行工具
- 超时后不假设副作用未发生，先用幂等键查询
- 工具输出按不可信数据处理，返回模型前限长并标注来源

### 可观测性与验收

![工具层应该按哪些阶段计量失败](./images/agent-03-09-tool-runtime-metrics.png)
...
1. 超时或网络中断时无法确定副作用是否已完成，幂等键可防止重复写入。
2. 可信运行时或工具网关，不是语言模型。
3. 不可信数据，其中的指令不得提升为系统指令。
