...
- 区分历史、状态、记忆、用户画像与 RAG
- 设计带来源和同意的记忆写入契约
- 测试检索、隔离、删除与遗忘

**贯穿场景**：Agent 记住用户偏好的输出语言，但不把一次性密钥或工具输出整段存入长期记忆。
...
![历史、运行状态、长期记忆、用户画像与 RAG 如何区分](./images/agent-05-01-memory-taxonomy.png)
...
![新旧记忆冲突时如何保留证据并解析](./images/agent-05-05-memory-conflict.png)
...
![一条信息在写入长期记忆前经过哪些门](./images/agent-05-02-memory-write-pipeline.png)
...
![删除请求如何覆盖主存储、索引与缓存](./images/agent-05-06-memory-deletion.png)
...
![检索结果如何带来源进入模型上下文](./images/agent-05-04-memory-retrieval.png)
...
![如何防止记忆跨用户或跨 Agent 泄漏](./images/agent-05-07-cross-user-isolation.png)
...
![一条可治理记忆必须包含哪些字段](./images/agent-05-03-memory-record-contract.png)

- 记忆条目包含 subject、content、source、created_at、confidence、scope 和 expires_at
- 写入前做敏感信息过滤、重要性判断和用户同意校验
- 检索结果返回来源和相似度，不把命中内容当作系统指令
...
![记忆系统应如何验证准确、时效与删除](./images/agent-05-08-memory-quality.png)
...
1. 用于评估可信度、处理冲突、解释输出与执行删除。
2. 主存储、向量/全文索引、缓存和可能的摘要派生物。
3. 不能；记忆是不可信上下文，权限低于系统和开发者指令。
...
06-Agent Skills：把可验证的过程知识从临时上下文中抽离出来。
