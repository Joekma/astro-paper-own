...
1. **服务器直接读取连接IP**：不再依赖 XFF 头，而是读取 `$remote_addr`
2. **反向代理验证**：只信任内网IP发送的 XFF
3. **双向TLS验证**：通过证书验证客户端身份
4. **IP信誉库**：识别数据中心IP和住宅IP

### 为什么 Nginx `$remote_addr` 无法伪造

TCP 连接建立过程中，IP 地址是在网络层（L3）确定的，HTTP 头部属于应用层（L7）。服务端读取的 `$remote_addr` 来自操作系统的 TCP/IP 协议栈，而非 HTTP 请求头。这意味着即使你伪造了所有 HTTP 头部，`$remote_addr` 依然是你真实的出口 IP，无法被篡改。

```
┌─────────────────────────────────────────────────────────────┐
│                      网络分层视角                            │
├─────────────────────────────────────────────────────────────┤
│  L7 应用层    │ HTTP headers (X-Forwarded-For) ← 可伪造      │
│              │                                              │
├─────────────────────────────────────────────────────────────┤
│  L3 网络层    │ IP 地址 (src/dst) ← 无法伪造                 │
│              │ $remote_addr 来自这里                          │
│              │                                              │
├─────────────────────────────────────────────────────────────┤
│  L2 数据链路层 │ MAC 地址 ← 无法伪造                         │
└─────────────────────────────────────────────────────────────┘
```
...
| 层面 | 能否伪造 | 说明 |
|------|----------|------|
| HTTP Header | ✅ 可以 | 应用层数据，客户端可随意构造 |
| IP 地址 | ❌ 不可以 | 依赖 TCP 三次握手，无法伪造来源 IP |
| TCP 握手 | ❌ 不可以 | 伪造 IP 需要发送 RST/ACK，但收不到响应 |
| $remote_addr | ❌ 不可以 | Nginx 读取的是系统级别的连接 IP |
...
print(response.json())
# 响应中的 origin 仍然是你的真实 IP，不是 1.2.3.4
```
...
```javascript
// 读取 HTTP 头部 —— 可以被伪造
const xff = req.headers['x-forwarded-for'];  // 可能是 1.2.3.4

// 读取 TCP 连接的真实 IP —— 无法伪造
const realIp = req.socket.remoteAddress;     // 永远是真实 IP

console.log('XFF:', xff);       // "1.2.3.4, 127.0.0.1"
console.log('真实IP:', realIp); // "203.0.113.42"
```

**结论**：如果你想真正改变 `$remote_addr`，唯一的办法是使用代理服务器或 VPN，让你的请求先经过中间节点，真实 IP 就变成了代理服务器的 IP。单纯伪造 HTTP 头部只能欺骗那些「只信任 XFF」的服务器。
